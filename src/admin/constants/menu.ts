...
export const SIDE_MENU_ITEMS: Record<string, { path: string; label: string }[]> = {
  ...
  stock: [
    ...
    { path: 'sales', label: '판매중' },
    { path: 'stop', label: '판매중지' },
    { path: 'package', label: '패키지' },
    { path: 'nets-scene', label: '넷츠 Scene' },
  ],
  sales: [
    { path: 'list', label: '판매 관리' },
    { path: 'ranking', label: '판매 순위' },
  ],
  account: [{ path: 'list', label: '정산 내역' }],
  member: [{ path: 'list', label: '회원 관리' }],
  board: [
    { path: 'list', label: '게시판 관리' },
    { path: 'contents', label: '게시글 관리' },
  ],
